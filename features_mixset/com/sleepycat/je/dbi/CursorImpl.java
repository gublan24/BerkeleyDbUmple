...
public class CursorImpl implements Cloneable {
   ...
   * Creates a cursor.
   * 
   * @param retainNonTxnLocksis
   *          true if non-transactional locks should be retained (notreleased
  ...
  /**
   * 
   * Advance a cursor. Used so that verify can advance a cursor even in the face
   * of an exception [12932].
   * 
   * @param keyon
   *          return contains the key if available, or null.
   * @param dataon
   *          return contains the data if available, or null.
   */
  // line 263 "../../../../CursorImpl.ump"
  ...
  public void addCursor(BIN bin) {
    if (bin != null) {
      Label208: assert bin.isLatchOwner();
      // this.hook208(bin);
      bin.addCursor(this);
    }
  }
  ...
  /**
   * 
   * Reset a cursor to an uninitialized state, but unlike close(), allow it to be
   * used further.
   */
  // line 396 "../../../../CursorImpl.ump"
  ...
  public OperationStatus delete(boolean b) throws DatabaseException {
    ...
    try {
      ...
      Label284: oldLNSize = ln.getMemorySizeIncludedByParent();
      ;
      long newLsn = ln.delete(database, lnKey, dupKey, oldLsn, locker);
      long newLNSize = 0;
      // newLNSize = this.hook283(ln, newLNSize);
      Label283: newLNSize = ln.getMemorySizeIncludedByParent();
  ...
  /**
   * 
   * Return a new copy of the cursor. If position is true, position the returned
  ...
  /**
   * 
   * Search for the next key (or duplicate) following the given key (and datum),
   * and acquire a range insert lock on it. If there are no more records following
   * the given key and datum, lock the special EOF node for the database.
   */
  // line 548 "../../../../CursorImpl.ump"
  public void lockNextKeyForInsert(DatabaseEntry key, DatabaseEntry data) throws DatabaseException {
    ...
    try {
      ...
      if ((searchResult & FOUND) != 0 && (searchResult & FOUND_LAST) == 0) {
        ...
        /*
         * If searchAndPosition found a record (other than the last one), in all cases
         * we should advance to the next record:
         *
         * 1- found a deleted record, 2- found an exact match, or 3- found the record
         * prior to the given key/data.
         *
         * If we didn't match the key, skip over duplicates to the next key with
         * getNextNoDup.
         */
   ...
   * present.
   * <p>
   * This method is called directly internally for putting tree map LNs and file
   * summary LNs. It should not be used otherwise, and in the future we should
   * find a way to remove this special case.
   * </p>
   */
  // line 607 "../../../../CursorImpl.ump"
  public OperationStatus putLN(byte[] key, LN ln, boolean allowDuplicates) throws DatabaseException {
    assert assertCursorState(false) : dumpToString(true);
    Label217: assert LatchSupport.countLatchesHeld() == 0;
    // original();
    ;// this.hook217();
   ...
   * @param data
   */
  // line 665 "../../../../CursorImpl.ump"
  public OperationStatus putCurrent(DatabaseEntry data, DatabaseEntry foundKey, DatabaseEntry foundData)
    ...
    try {
      ...
      Label285: targetBin.updateEntry(targetIndex, newLsn, oldLNSize, newLNSize);
      Label269: releaseBINs();
      // original();
      ;// this.hook269();
   ...
   */
  // line 777 "../../../../CursorImpl.ump"
  public OperationStatus getCurrent(DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType)
      throws DatabaseException {
    assert assertCursorState(true) : dumpToString(true);
    if (bin == null) {
      return OperationStatus.KEYEMPTY;
    }
    Label220: if (dupBin == null) {
      latchBIN();
    } else {
      latchDBIN();
    }
  ...
  /**
   * 
   * Retrieve the current record. Assume the bin is already latched. Return with
   * the target bin unlatched.
   */
  // line 791 "../../../../CursorImpl.ump"
  public OperationStatus getCurrentAlreadyLatched(DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType,
  ...
  /**
   * 
   * Retrieve the current LN, assuming the BIN is already latched. Return with the
   * target BIN unlatched.
   */
  // line 819 "../../../../CursorImpl.ump"
  public LN getCurrentLNAlreadyLatched(LockType lockType) throws DatabaseException {
    try {
      Label223: assert assertCursorState(true) : dumpToString(true);
      Label272: assert checkAlreadyLatched(true) : dumpToString(true);
      // original();
  ...
  // line 847 "../../../../CursorImpl.ump"
  public OperationStatus getNext(DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType, boolean forward,
   ...
   * 
   * @param foundKeyDatabaseEntry
   *          to use for returning key
   * @param foundDataDatabaseEntry
   *          to use for returning data
   * @param forwardif
   *          true, move forward, else move backwards
   * @param alreadyLatchedif
   *          true, the bin that we're on is already latched.
   * @return the status and an indication of whether we advanced to a new
   *         keyduring the operation.
   */
  // line 860 "../../../../CursorImpl.ump"
  public KeyChangeStatus getNextWithKeyChangeStatus(DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType,
      boolean forward, boolean alreadyLatched) throws DatabaseException {
    assert assertCursorState(true) : dumpToString(true);
    Label224: assert checkAlreadyLatched(alreadyLatched) : dumpToString(true);
    ...
    try {
      while (bin != null) {
        ...
        Label225: assert checkAlreadyLatched(alreadyLatched) : dumpToString(true);
        if (!alreadyLatched) {
          latchBIN();
        } else {
        ...
        if ((forward && ++index < bin.getNEntries()) || (!forward && --index > -1)) {
          ...
          if (ret == OperationStatus.SUCCESS) {
            ...
            Label227: assert LatchSupport.countLatchesHeld() == 0;
            // original();
            // this.hook227();
      ...
      Label231: assert LatchSupport.countLatchesHeld() == 0 : LatchSupport.latchesHeldToString();
      // original();
      // this.hook231();
      if (binToBeRemoved != null) {
        flushBINToBeRemoved();
  ...
  private void flushBINToBeRemoved() throws DatabaseException {
    ...
    Label232: binToBeRemoved.releaseLatch();
    // original();
    ;// this.hook232();
    binToBeRemoved = null;
  }

  // line 942 "../../../../CursorImpl.ump"
  public OperationStatus getNextNoDup(DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType,
      boolean forward, boolean alreadyLatched) throws DatabaseException {
    assert assertCursorState(true) : dumpToString(true);
    if (dupBin != null) {
   ...
   */
  // line 955 "../../../../CursorImpl.ump"
  public OperationStatus getFirstDuplicate(DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType)
      throws DatabaseException {
    assert assertCursorState(true) : dumpToString(true);
    if (dupBin != null) {
      removeCursorDBIN();
  ...
  /**
   * 
   * Enter with dupBin unlatched. Pass foundKey == null to just advance cursor to
   * next duplicate without fetching data.
   */
  // line 969 "../../../../CursorImpl.ump"
  public OperationStatus getNextDuplicate(DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType,
      boolean forward, boolean alreadyLatched) throws DatabaseException {
    assert assertCursorState(true) : dumpToString(true);
    Label250: assert checkAlreadyLatched(alreadyLatched) : dumpToString(true);
    ;
    try {
      while (dupBin != null) {
        Label251: if (!alreadyLatched) {
          latchDBIN();
        } else {
          alreadyLatched = false;
        }
        ...
        if ((forward && ++dupIndex < dupBin.getNEntries()) || (!forward && --dupIndex > -1)) {
          ...
          if (ret == OperationStatus.SUCCESS) {
            ...
            Label257: assert LatchSupport.countLatchesHeld() == 0;
            ;
          ...
          /*
           * We need to go to the next DBIN. Remove the cursor and be sure to change the
           * dupBin field after removing the cursor.
           */
          if (dupBinToBeRemoved != null) {
            flushDBINToBeRemoved();
          }
          ...
          dupBin = null;
          Label255: dupBinToBeRemoved.releaseLatch();
          ;
          ...
          if (treeStatsAccumulator != null) {
            ...
            try {
              ...
              try {
                ...
                Label201_1: duplicateRoot.releaseLatch();
                ;
              }
            } finally {
          ...
          Label254: assert (LatchSupport.countLatchesHeld() == 0);
          ...
          if (forward) {
            newDupBin = (DBIN) database.getTree().getNextBin(dupBinToBeRemoved, true /* traverseWithinDupTree */);
          } else {
            newDupBin = (DBIN) database.getTree().getPrevBin(dupBinToBeRemoved, true /* traverseWithinDupTree */);
          }
          ...
          if (newDupBin == null) {
             ...
             */
            dupBin = newDupBin;
            Label256: alreadyLatched = true;
            ;
          }
        }
      }
    } finally {
      Label253: assert LatchSupport.countLatchesHeld() == 0;
      ;
      if (dupBinToBeRemoved != null) {
        flushDBINToBeRemoved();
      }
  ...
  private void flushDBINToBeRemoved() throws DatabaseException {
    ...
    Label233: dupBinToBeRemoved.releaseLatch();
    // original();
    // this.hook233();
  ...
  /**
   * 
   * Position the cursor at the first or last record of the database. It's okay if
   * this record is deleted. Returns with the target BIN latched.
   * 
   * @return true if a first or last position is found, false if the treebeing
   *         searched is empty.
   */
  // line 1073 "../../../../CursorImpl.ump"
   ...
   * bitwise or'ed into the int. We find out if there was any kind of match and if
   * the match was exact. Note that this match focuses on whether the searching
   * criteria (key, or key and data, depending on the search type) is met.
   * <p>
   * Note this returns with the BIN latched!
   * </p>
   * <p>
   * If this method returns without the FOUND bit set, the caller can assume that
   * no match is possible. Otherwise, if the FOUND bit is set, the caller should
   * check the EXACT_KEY and EXACT_DATA bits. If EXACT_KEY is not set (or for BOTH
   * and BOTH_RANGE, if EXACT_DATA is not set), an approximate match was found. In
   * an approximate match, the cursor is always positioned before the target
   * key/data. This allows the caller to perform a 'next' operation to advance to
   * the value that is equal or higher than the target key/data.
   * </p>
   * <p>
   * Even if the search returns an exact result, the record may be deleted. The
   * caller must therefore check for both an approximate match and for whether the
   * cursor is positioned on a deleted record.
   * </p>
   * <p>
   * If SET or BOTH is specified, the FOUND bit will only be returned if an exact
   * match is found. However, the record found may be deleted.
   * </p>
   * <p>
   * There is one special case where this method may be called without checking
   * the EXACT_KEY (and EXACT_DATA) bits and without checking for a deleted
   * record: If SearchMode.SET is specified then only the FOUND bit need be
   * checked. When SET is specified and FOUND is returned, it is guaranteed to be
   * an exact match on a non-deleted record. It is for this case only that this
   * method is public.
   * </p>
   * <p>
   * If FOUND is set, FOUND_LAST may also be set if the cursor is positioned on
   * the last record in the database. Note that this state can only be counted on
   * as long as the BIN is latched, so it is not set if this method must release
   * the latch to lock the record. Therefore, it should only be used for
   * optimizations. If FOUND_LAST is set, the cursor is positioned on the last
   * record and the BIN is latched. If FOUND_LAST is not set, the cursor may or
   * may not be positioned on the last record. Note that exact searches always
   * perform an unlatch and a lock, so FOUND_LAST will only be set for inexact
   * (range) searches.
   * </p>
   * <p>
   * Be aware that when an approximate match is returned, the index or dupIndex
   * may be set to -1. This is done intentionally so that a 'next' operation will
   * increment it.
   * </p>
   */
  // line 1142 "../../../../CursorImpl.ump"
  public int searchAndPosition(DatabaseEntry matchKey, DatabaseEntry matchData, SearchMode searchMode,
  ...
  /**
   * 
   * For this type of search, we need to match both key and data. This method is
   * called after the key is matched to perform the data portion of the match. We
   * may be matching just against an LN, or doing further searching into the dup
   * tree. See searchAndPosition for more details.
   */
  // line 1213 "../../../../CursorImpl.ump"
  ...
  /**
   * 
   * Locks the given LN's node ID; a deleted LN will not be locked or returned.
   * Attempts to use a non-blocking lock to avoid unlatching/relatching. Retries
   * if necessary, to handle the case where the LN is changed while the BIN is
   * unlatched. Preconditions: The target BIN must be latched. When positioned in
   * a dup tree, the BIN may be latched on entry also and if so it will be latched
   * on exit. Postconditions: The target BIN is latched. When positioned in a dup
   * tree, the BIN will be latched if it was latched on entry or a blocking lock
   * was needed. Therefore, when positioned in a dup tree, releaseDBIN should be
   * called.
   * 
   * @param lnthe
   *          LN to be locked.
   ...
   * @return the LockResult containing the LN that was locked, or containing anull
   *         LN if the LN was deleted or cleaned. If the LN is deleted, a lock
   *         will not be held.
   */
  // line 1273 "../../../../CursorImpl.ump"
   ...
   *          type of lock requested.
   * @return the LockResult containing the LN that was locked, or containing anull
   *         LN if the LN was cleaned.
   */
  // line 1292 "../../../../CursorImpl.ump"
   ...
   * non-blocking lock to avoid unlatching/relatching. Preconditions: The dupRoot,
   * BIN and DBIN are latched. Postconditions: The dupRoot, BIN and DBIN are
   * latched. Note that the dupRoot may change during locking and should be
   * refetched if needed.
   * 
   * @param dupRootthe
   *          duplicate root containing the DupCountLN to be locked.
   * @param lockTypethe
   *          type of lock requested.
   * @return the LockResult containing the LN that was locked.
   */
  // line 1331 "../../../../CursorImpl.ump"
   ...
   * position. Preconditions: The BIN must be latched and the current BIN entry
   * must contain a DIN. Postconditions: The BIN and DIN will be latched. The DBIN
   * will remain latched if isDBINLatched is true.
   * 
   * @param isDBINLatchedis
   *          true if the DBIN is currently latched.
   */
  // line 1355 "../../../../CursorImpl.ump"
  public DIN getLatchedDupRoot(boolean isDBINLatched) throws DatabaseException {
    assert bin != null;
    Label243: assert bin.isLatchOwner();
    // original();
    // this.hook243();
    assert index >= 0;
    DIN dupRoot = (DIN) bin.fetchTarget(index);
    Label242: if (isDBINLatched) {
  ...
  /**
   * 
   * Return this lock to its prior status. If the lock was just obtained, release
   * it. If it was promoted, demote it.
   */
  // line 1426 "../../../../CursorImpl.ump"
  ...
  /**
   * 
   * @throws RunRecoveryExceptionif
   *           the underlying environment is invalid.
   */
  // line 1451 "../../../../CursorImpl.ump"
  public void checkEnv() throws RunRecoveryException {
    database.getDbEnvironment().checkIfInvalid();
  }

  // line 1455 "../../../../CursorImpl.ump"
  ...
  // line 1463 "../../../../CursorImpl.ump"
  public void setLockerPrev(CursorImpl p) {
    lockerPrev = p;
  }

  // line 1467 "../../../../CursorImpl.ump"
  ...
  /**
   * 
   * Dump the cursor for debugging purposes. Dump the bin and dbin that the cursor
   * refers to if verbose is true.
   */
  // line 1474 "../../../../CursorImpl.ump"
   ...
   */
  // line 1481 "../../../../CursorImpl.ump"
  public void dump() {
    System.out.println(dumpToString(true));
  }

  // line 1485 "../../../../CursorImpl.ump"
   ...
   * For example, a cursor used to implement eviction should not itself perform
   * eviction. Eviction is enabled by default.
   */
  ...
  /**
   * 
   * Evict the LN node at the cursor position. This is used for internal databases
   * only.
   */
  ...
  /* PLEASE DO NOT EDIT THIS CODE */
  /*
   * This code was generated using the UMPLE 1.29.1.4260.b21abf3a3 modeling
   * language!
   */
  ...
  public static class CursorImpl_getNextDuplicate {
    ...
    public CursorImpl_getNextDuplicate(CursorImpl _this, DatabaseEntry foundKey, DatabaseEntry foundData,
        LockType lockType, boolean forward, boolean alreadyLatched) {
      this._this = _this;
  ...
  // line 90 "../../../../Latches_CursorImpl_inner.ump"
  public static class CursorImpl_fetchCurrent {
    ...
    public CursorImpl_fetchCurrent(CursorImpl _this, DatabaseEntry foundKey, DatabaseEntry foundData, LockType lockType,
        boolean first) {
      this._this = _this;
      this.foundKey = foundKey;
      this.foundData = foundData;
    ...
    public OperationStatus execute() throws DatabaseException {
      try {
        ...
        Label259: assert _this.targetBin.isLatchOwner();
        // original();
        // this.hook259();
        n = null;
        if (_this.targetIndex < 0 || _this.targetIndex >= _this.targetBin.getNEntries()
          ...
          if (_this.targetBin.isEntryPendingDeleted(_this.targetIndex)) {
            Label280: envImpl = _this.database.getDbEnvironment();
        ...
        if (n == null) {
          ...
          Label261: _this.targetBin.releaseLatchIfOwner();
          // original();
          // this.hook261();
          return OperationStatus.KEYEMPTY;
        }
