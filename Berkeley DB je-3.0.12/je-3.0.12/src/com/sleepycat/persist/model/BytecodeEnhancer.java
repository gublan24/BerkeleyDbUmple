 ...
 *
 * $Id: BytecodeEnhancer.java,v 1.4 2006/05/01 05:16:01 mark Exp $
 */
...
import static com.sleepycat.asm.Opcodes.*;
...
class BytecodeEnhancer extends ClassAdapter {
    ...
    private void genBdbReadPriKeyField() {
        ...
        if (priKeyField != null) {
            if (!genReadSimpleKeyField(mv, priKeyField)) {
                ...
                mv.visitVarInsn(ALOAD, 2);
                mv.visitMethodInsn
                    (INVOKEVIRTUAL, "com/sleepycat/persist/impl/EntityInput",
                     "readKeyObject",
     ...
     *      super.bdbReadSecKeyFields(...); // if has super
     *      secKeyField1 = input.readInt();
     *      secKeyField2 = (String) input.readObject();
     *      // etc
     *  }
     *
     *  Until adding class evolution, ignore startField/endField/superLevel.
     */
    private void genBdbReadSecKeyFields() {
        ...
        if (hasPersistentSuperclass) {
            ...
            mv.visitMethodInsn
                (INVOKESPECIAL, superclassName, "bdbReadSecKeyFields",
    ...
    private void genRegisterPrimaryKey(MethodVisitor mv, boolean input) {
        ...
        mv.visitFieldInsn
            (GETFIELD, className, priKeyField.name,
             priKeyField.type.getDescriptor());
        mv.visitMethodInsn
            (INVOKEVIRTUAL, entityInputOrOutputClass, "registerPriKeyObject",
             "(Ljava/lang/Object;)V");
    }
     ...
     *                                  int endField,
     *                                  int superLevel) {
     *      super.bdbReadNonKeyFields(...); // if has super
    ...
    private void genBdbReadNonKeyFields() {
        ...
        if (hasPersistentSuperclass) {
            ...
            mv.visitVarInsn(ILOAD, 4);
            mv.visitMethodInsn
                (INVOKESPECIAL, superclassName, "bdbReadNonKeyFields",
                 "(Lcom/sleepycat/persist/impl/EntityInput;III)V");
        }
        if (isCompositeKey) {
            for (FieldInfo field : nonKeyFields) {
    ...
    private void genWriteField(MethodVisitor mv, FieldInfo field) {
        ...
        if (sort == Type.OBJECT || sort == Type.ARRAY) {
            mv.visitInsn(ACONST_NULL);
            mv.visitMethodInsn
                (INVOKEVIRTUAL, "com/sleepycat/persist/impl/EntityOutput",
                 "writeObject",
