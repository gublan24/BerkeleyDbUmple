 ...
 *
 * $Id: Checkpointer.java,v 1.131 2006/05/06 01:29:42 linda Exp $
 */
...
public class Checkpointer extends DaemonThread {
    ...
    private static void flushDirtyNodes(EnvironmentImpl envImpl,
        ...
        /* 
         * In general, we flush until we reach the maxFlushLevel. If
         * we're cleaning deferred write dbs, we sync only those dbs
         * all the way up to the root. onlyFlushDeferredWriteDbs is
         * true when we're above maxFlushLevel, but are still syncing.
         */
        ...
        while (dirtyMap.getNumLevels() > 0) {
            ...
            while (iter.hasNext()) {
                ...
                /* 
                 * Flush if we're below maxFlushLevel, or we're above
                 * and syncing cleaned deferred write dbs.
                 */
            ...
            if (result.parent != null) {
                ...
                try {
                    ...
                    if (mustLogParent) {
                        ...
                        dirtyMap.addDirtyIN(result.parent);
                    }
                } finally {
